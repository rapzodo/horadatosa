...
public class AnimalCRUDService{
	...
	private BaseMongoDao<DomainSuperClass> dao= new DaoFactory<DomainSuperClass>().getDao(Animal.class);
	
	
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public List<Animal> getAll(){
		return (List<Animal>) bean.listAll(dao);
	}
	
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("regex/{field}/{regex}")
	public List<Animal> getByRegex(@PathParam("field")String field, @PathParam("regex")String regex){
		return (List<Animal>) bean.getByRegex(dao,field, regex);
	}
	
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("{id}")
	public Animal getById(@PathParam("id") String id){
		return (Animal) bean.getById(dao,id);
	}
	...
	public Response save(Animal Animal){
		return Response.ok(bean.saveOrUpdate(dao,Animal)).build();
	}
